...
SerialPort = SP.SerialPort;
	...
	var rgbPort = new SerialPort("/dev/tty.usbmodemfa131", {
	  baudrate: 9600
	}, false);
	...
	//	var polarY = toPolar( Math.cos(y)*r2 + Math.cos(y)*Math.cos(x)*r1, 120 + Math.sin(y)*r2 + Math.sin(y)*Math.cos(x)*r1 );
		...
		var rotation = 0; //Math.floor(-x*.28) + Math.floor(w*.28/2);//Math.floor(0 + Math.sin(x)*r);//Math.floor(180 * Math.random() - 90);
		var stretch = 80;//-x + 50 + w;//Math.floor(210 * Math.random());
		var height = 100; //Math.floor((-y + 5 + h) + (x/2)%2 * 6 - 3  ) ;
		...
		var r = 255;// /80 //Math.floor(Math.random()*255);
		var g = 255;// /35 // Math.floor(Math.random()*255);
		var b = 255;// /50 // Math.floor(Math.random()*255);
