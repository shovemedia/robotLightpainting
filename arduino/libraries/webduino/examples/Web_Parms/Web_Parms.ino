...
static uint8_t mac[] = { 0x0A, 0x0B, 0x0C, 0x0D, 0x0E, 0x0F };
...
void onCmd(WebServer &server, WebServer::ConnectionType type, char *url_tail, bool tail_complete)
{
  ...
  switch (type)
    ...
    digitalWrite(offPin, LOW);   // sets the LED on
    digitalWrite(onPin, HIGH);   // sets the LED on
    delay(1000);                  // waits for a second
  
    digitalWrite(offPin, LOW);   // sets the LED on
    digitalWrite(onPin, LOW);   // sets the LED on
}
...
void offCmd(WebServer &server, WebServer::ConnectionType type, char *url_tail, bool tail_complete)
{
  ...
  switch (type)
    ...
    digitalWrite(offPin, HIGH);   // sets the LED on
    digitalWrite(onPin, LOW);   // sets the LED on
    delay(1000);                  // waits for a second
