...
void UF_uArm::setPosition(double _stretch, double _height, int _armRot, int _handRot)
{
	...
	dStretch = abs(angleR - stretchLst);
	dHeight = abs(angleL - heightLst);
	dRot = abs(_armRot - armRotLst);
	
	double diag = sqrt(dStretch*dStretch + dHeight*dHeight + dRot*dRot);
	//double dMax= max(max(dHeight/heightRange, dRot/rotRange), (dStretch/stretchRange));

//1 * servoSpd;//
	servoSpdR = floor(((double) dStretch /diag) * (double) servoSpd);//servoSpd;
	servoSpdL = floor(((double) dHeight /diag) * (double) servoSpd);//servoSpd;
	servoSpdRot = floor(((double) dRot /diag) * (double) servoSpd);//servoSpd;
...
void UF_uArm::setServoSpeeds(unsigned char _servoSpeed) // 0=full speed, 1-255 slower to faster
{
	servoSpd = _servoSpeed;
}
